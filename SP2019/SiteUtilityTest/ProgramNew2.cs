...
namespace SiteUtilityTest
{
    public class ProgramNew2
    {
        ...
        private static void Init_ModifyView_IWH(string wUrl)
        {
            ...
            try
            {
                SiteLogUtility.Log_Entry("ModifyView for DataExchange Iwh...", true);
                modifyView(wUrl, slu.pageNameDataExchange + ".aspx", slu.webpartDataExchangeIwh);

                SiteLogUtility.Log_Entry("ModifyView for RiskAdjustment Iwh...", true);
                modifyView(wUrl, slu.pageNameRiskAdjustment + ".aspx", slu.webpartRiskAdjustmentIwh);

                SiteLogUtility.Log_Entry("ModifyView for Quality Iwh...", true);
                modifyView(wUrl, slu.pageNameQuality + ".aspx", slu.webpartQualityIwh);
            }
            catch (Exception ex)
            {
        ...
        private static void Init_ModifyView_CKCC(string wUrl)
        {
            ...
            try
            {
                SiteLogUtility.Log_Entry("ModifyView for DataExchange CKCC...", true);
                modifyView(wUrl, slu.pageNameDataExchange + ".aspx", slu.webpartDataExchangeCkcc);

                SiteLogUtility.Log_Entry("ModifyView for RiskAdjustment CKCC...", true);
                modifyView(wUrl, slu.pageNameRiskAdjustment + ".aspx", slu.webpartRiskAdjustmentCkcc);

                SiteLogUtility.Log_Entry("ModifyView for Quality CKCC...", true);
                modifyView(wUrl, slu.pageNameQuality + ".aspx", slu.webpartQualityCkcc);
            }
            catch (Exception ex)
            {
        ...
        private static void Init_Benefit(PracticeSite practiceSite)
        {
            ...
            bool ConfigSuccess = false;
            string LayoutsFolderMnt = string.Empty;
        ...
        public static DataTable readPMSiteData(string strPM)
        {
            try
            {
                ...
                string query = @"SELECT * FROM [HealthCloud_NightlyProd].[PORTAL].[vwPracticeInfo] WHERE GroupID = " + strPM + " ORDER BY GroupID";
        ...
        public static void uploadMultiPartSupportingFilesAll(string wUrl, PracticeSite practiceSite)
        {
            string LayoutsFolderMnt = @"C:\Projects\PracticeSite-Core\Dev\PracticeSiteTemplate\Config\";
            try
            {
        ...
        private static DataTable dbReadPMSiteData(string strPM)
        {
            try
            {
                DataTable dataTable = readPMSiteData(strPM);
                return dataTable;
            }
            catch (Exception ex)
            {
                SiteLogUtility.CreateLogEntry("dbReadPMSiteData", ex.Message, "Error", "");
                return null;
            }
        }
        private static bool DataTableToCsv(string strPM)
        {
        ...
        public static void CheckIfPageExists(string webURL, string pageName)
        {
            ...
            using (ClientContext clientContext = new ClientContext(webURL))
            {
                try
                {
                    ...
                    {
                        SiteLogUtility.Log_Entry($"File Does NOT Exist: {pageName}.aspx", true);
                        return;
                    }
                    ...
                    clientContext.Load(file);
                    clientContext.ExecuteQuery();
                    SiteLogUtility.Log_Entry($"File Exists: {file.Name}", true);


                    var listTitle = "Pages";
                    var list = clientContext.Web.Lists.GetByTitle(listTitle);
                    var items = list.GetItems(CamlQuery.CreateAllItemsQuery());
                    clientContext.Load(items, icol => icol.Include(i => i["FileRef"], i => i.ContentType));
                    clientContext.ExecuteQuery();
                    foreach (var item in items)
                    {
        ...
        public static void PrintWebPartProperties(string webURL, string pageName)
        {
            ...
            using (ClientContext clientContext = new ClientContext(webURL))
            {
                try
                {
                    ...
                    if (wpManager.WebParts.Count > 0)
                    {
                        foreach (WebPartDefinition webPartDef in wpManager.WebParts)
                        {
                            clientContext.Load(webPartDef.WebPart, wp => wp.Title, wp => wp.TitleUrl, wp => wp.ZoneIndex);
                            clientContext.Load(webPartDef.WebPart.Properties);
                            clientContext.ExecuteQuery();

                            SiteLogUtility.Log_Entry("\n", true);
                            SiteLogUtility.Log_Entry(webPartDef.WebPart.Title.ToString(), true);
                            SiteLogUtility.Log_Entry("ZoneIndex: " + webPartDef.WebPart.ZoneIndex.ToString(), true);

                            var propValues = webPartDef.WebPart.Properties.FieldValues;

                            List<string> listWP = new List<string>();
                            foreach (var property in propValues)
                            {
                                switch (property.Key)
                                {
                                    case "Title":
                                        listWP.Insert(0, "Title: " + property.Value);
                                        break;
                                    case "Height":
                                    case "Width":
                                    case "ContentLink":
                                    ...
                                    case "ListUrl":
                                    case "XmlDefinition":
                                        listWP.Add(property.Key + ":  " + property.Value);
                                        break;
                                }
                            }

                            foreach (var sortListWP in listWP)
                            {
                                SiteLogUtility.Log_Entry(sortListWP.ToString(), true);
                            }
                        }
                    }
